...
setup_stack (void **esp, const char *file_name) 
{
  ...
  * Create a new page, if unsuccessful free new_page, arguments, and copy */
  ...
  /* Upon success, initialize page and get a frame */
  page_init (cur, new_page, PHYS_BASE - PGSIZE, 0, NULL, 0, 0, true);
  get_frame (new_page);

  kpage = new_page->frame->physical_address;
  // kpage = palloc_get_page (PAL_USER | PAL_ZERO); // Initially here
  if (kpage != NULL) 
    {
